"""
backend.py — Clean version, no column dependencies
Season detection purely by date comparison inline
"""
...
# ── season mask: inline, never stored as column ──
def season_mask(df):
    return (df["Date"] >= SEASON_START) & (df["Date"] <= SEASON_END)
...
def get_all_clubs(players_dict, season="2024-25"):
    ...
    for df in players_dict.values():
        clubs.update(df[season_mask(df)]["Team"].dropna().unique())
    return sorted(clubs)
...
def get_squad_with_positions(players_dict, club, season="2024-25"):
    ...
    for name, df in players_dict.items():
        club_df = df[season_mask(df) & (df["Team"] == club)]
        if len(club_df) > 0:
...
def get_players_by_group(players_dict, club, group, season="2024-25"):
    ...
    return sorted(filtered) if filtered else sorted(squad.keys())
...
def build_training_data(players_dict, club, season="2024-25"):
    club_matches = []
    for name, df in players_dict.items():
        cdf = df[season_mask(df) & (df["Team"] == club)]
